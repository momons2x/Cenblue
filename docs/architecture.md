...
Scheduling treats `APP_TIMEZONE` as the canonical business timezone. Schedule controls use native date and time inputs; the server converts that wall time through Temporal into an absolute instant and rejects invalid daylight-saving transitions. Unscheduled approved jobs remain manual-only, while automatic owners claim only due jobs with non-null schedules. For due jobs that auto-publishing cannot run, the dashboard surfaces an inline warning identifying the blocking condition. Bulk approval validates all selected jobs in one database transaction.
