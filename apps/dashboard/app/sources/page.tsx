...
export default async function SourcesPage({ searchParams }: { searchParams: Promise<{ bookmarkInserted?: string; bookmarkDuplicates?: string; bookmarkError?: string }> }) {
  ...
  const collectors = settings.identities.filter((identity) => identity.role === "COLLECTOR" && identity.enabled);
    ...
    <div className="page-grid">
      <section className="panel form-panel">
        ...
        <ActionForm action={addSource} className="stack-form">
          ...
          <label>Collector identity<select name="collectorIdentityId" required>{collectors.map((identity) => <option key={identity.id} value={identity.id}>{identity.label} (@{identity.expectedUsername})</option>)}</select></label>
      ...
      <section className="panel">
        ...
        {active.length === 0 ? <Empty>Add or restore a source account to begin collecting.</Empty> : <div className="source-list">{active.map((source) =>
            ...
            <ActionForm action={assignSourceCollector} className="inline-form"><input type="hidden" name="sourceId" value={source.id} /><select name="collectorIdentityId" defaultValue={source.collectorIdentityId ?? ""} aria-label="Collector identity" required>{collectors.map((identity) => <option key={identity.id} value={identity.id}>{identity.label}</option>)}</select><SubmitButton>Assign</SubmitButton></ActionForm>
