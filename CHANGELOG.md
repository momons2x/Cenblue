...
- Batch scheduling now auto-spills: batches larger than one day's active window continue into the following days (same active hours, quiet hours stay empty) until every selected post has a time, with an optional per-day post cap; batch-schedule submissions also now include the selected review items, fixing a validation error that always rejected the action.
- The batch-schedule active window may now cross midnight (end earlier than start), so posts can be scheduled overnight with the daytime kept quiet; start and end must differ, which Settings now enforces.
